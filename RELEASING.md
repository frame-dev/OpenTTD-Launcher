...
1. Confirm the POM version and release notes. Run `pwsh -File scripts/package-release.ps1` with PowerShell 7, Java 21+, and Maven 3.9+. It runs tests and creates binary/source ZIPs and SHA-256 checksums under `target/release`.
...
3. Commit the intended changes and tag that exact commit `v1.0.0`, matching the POM version. Upload both ZIPs and `SHA256SUMS.txt` together to a draft GitHub release. Use `RELEASE_NOTES.md` as its description.
